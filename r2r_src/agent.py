...
class BaseAgent(object):
    ...
    def test(self, iters=None, **kwargs):
        ...
        self.loss = 0
        if args.tree != 'none':
            roll_out_func = self.rollout_graph
        else:
            roll_out_func = self.rollout
        if iters is not None:
            # For each time, it will run the first 'iters' iterations. (It was shuffled before)
            for i in range(iters):
                for traj in roll_out_func(**kwargs):
                    self.loss = 0
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        if args.tree == 'dep':
            self.encoder = model.TreeLSTM(tok.vocab_size(), args.wemb, args.rnn_dim, args.dropout, cell_type='childsum').cuda()
        else:
            self.encoder = model.EncoderLSTM(tok.vocab_size(), args.wemb, enc_hidden_size, padding_idx,
                                         args.dropout, bidirectional=args.bidir, num_layers=args.lstm_layer).cuda()
    ...
    def _sort_batch_graph(self, obs):
        seq_length = []
        graph_tensor = []
        leaves_length = []
        max_length = 0
        for ob in obs:
            max_length = max(ob['graph'].number_of_nodes(), max_length)
            seq_length.append(ob['graph'].number_of_nodes())
            leaves_length.append(ob['num_leaves'])
        for i, ob in enumerate(obs):
            n = max_length - ob['graph'].number_of_nodes()
            add_graph = copy.deepcopy(ob['graph'])
            add_graph.add_nodes(n, {'x': torch.ones((n), dtype=torch.int64) * self.tok.word_to_index['<PAD>'], 'leaf': torch.zeros((n), dtype=torch.int64), 'npad': torch.zeros((n), dtype=torch.int64)})
            assert ob['graph'].number_of_nodes() == seq_length[i]
            graph_tensor.append(add_graph)

        graph = dgl.batch(graph_tensor)

        mask = utils.length2mask(seq_length)

        return graph, seq_length, mask.cuda(), leaves_length
    ...
    def _candidate_variable_shortest(self, obs, actions):
        ...
        for i, (ob, act) in enumerate(zip(obs, actions)):
            if act == -1 or act == args.ignoreid or act == len(ob['candidate']):  # Ignore or Stop --> Just use zero vector as the feature
                pass
            else:
                c = ob['candidate'][act]
    ...
    def from_shortest_path(self, viewpoints=None, get_first_feat=False):
        ...
        while not ended.all():
            ...
            can_feats.append(self._candidate_variable_shortest(obs, teacher_action))
    ...
    def get_distance_baseline(self, a_t, obs):
        batch_size = len(obs)
        ...
        for i in range(batch_size):
            ...
            if action != -1:
                ...
                src_point = obs[i]['viewpoint']
                trg_point = select_candidate['viewpointId']
                dist[i] = self.env.distances[obs[i]['scan']][src_point][trg_point]
            else:
                dist[i] = obs[i]['distance']
        return dist
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        parse_mask, phrase_len, phrase_mask = None, None, None
        ...
        for t in range(self.episode_len):
            ...
            # Agent
            h_t, c_t, logit, h1, sf_attn = self.decoder(input_a_t, f_t, candidate_feat,
                                                   h_t, h1, c_t,
                                                   ctx, ctx_mask,
        ...
        if train_ml is not None:
            ...
            self.logs['ml_loss'].append(ml_loss * train_ml / batch_size)
    ...
    def rollout_graph(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        """
        :param train_ml:    The weight to train with maximum likelihood
        :param train_rl:    whether use RL in training
        :param reset:       Reset the environment
        ...
        graph, seq_length, ctx_mask, leaves_length = self._sort_batch_graph(obs)


        h_init = torch.zeros((graph.number_of_nodes(), args.rnn_dim))
        c_init = torch.zeros((graph.number_of_nodes(), args.rnn_dim))
        ctx, decoder_init, c_t = self.encoder(graph, h_init, c_init, seq_length, leaves_length)
        ...
        for t in range(self.episode_len):
            ...
            h_t, c_t, logit, h1, sf_attn = self.decoder(input_a_t, f_t, candidate_feat, h_t, h1, c_t,
                                                                                       ctx, ctx_mask=ctx_mask,
                                                                                       already_dropfeat=(
                                                                                                   speaker is not None))
            hidden_states.append(h_t)
            ...
            # Make action and get the new state
            self.make_equiv_action(cpu_a_t, obs, traj=traj)
            obs = np.array(self.env._get_obs())
    ...
    def accumulate_gradient(self, feedback='teacher', **kwargs):
        ...
        if feedback == 'teacher':
            ...
            roll_out_func(train_ml=args.teacher_weight, train_rl=False, **kwargs)
        elif feedback == 'sample':
            self.feedback = 'teacher'
            roll_out_func(train_ml=args.ml_weight, train_rl=False, **kwargs)
            self.feedback = 'sample'
            roll_out_func(train_ml=None, train_rl=True, **kwargs)
        else:
            assert False

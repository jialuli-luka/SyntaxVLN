...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        for split in splits:
            for item in load_datasets([split]):
                ...
                if args.dataset == 'RXR':
                    if item['language'].split('-')[0] != args.language:
                        continue
                for j,instr in enumerate(item['instructions']):
                    ...
                    if tokenizer:
                        if args.language != 'en':
                            new_item['instr_encoding'] = tokenizer.encode_sentence_hite(new_item['parse_tree_origin'][j])
                        else:
                            new_item['instr_encoding'] = tokenizer.encode_sentence(instr)
                        if args.tree == 'dep':
                            parse = item['parse_tree_origin'][j]
                            new_item['graph'], new_item['num_leaves'] = tokenizer.encode_graph_dep(parse)
                        else:
                            new_item['parse_mask'] = np.array([False] * args.maxInput)
                            new_item['phrase_len'] = np.zeros(args.maxInput)
                            new_item['phrase_mask'] = np.zeros((args.maxInput, args.maxInput))
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if 'phrase_len' in item:
                obs[-1]['phrase_len'] = item['phrase_len']
            if 'parse_mask' in item:
                obs[-1]['parse_mask'] = item['parse_mask']
            if 'phrase_mask' in item:
                obs[-1]['phrase_mask'] = item['phrase_mask']
            if 'graph' in item:
                obs[-1]['graph'] = item['graph']
                obs[-1]['num_leaves'] = item['num_leaves']
            if 'parse_tree_origin' in item:
                obs[-1]['parse_tree_origin'] = item['parse_tree_origin']
            if 'graph_dep' in item:
                obs[-1]['graph_dep'] = item['graph_dep']
                obs[-1]['num_leaves_dep'] = item['num_leaves_dep']
            if 'parse_tree_origin_dep' in item:
                obs[-1]['parse_tree_origin_dep'] = item['parse_tree_origin_dep']

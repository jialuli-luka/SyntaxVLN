...
class SelfAttention(nn.Module):
    ...
    def __init__(self, query_dim, ctx_dim, attn_dropout=0.1):
        '''Initialize layer.'''
        super(SelfAttention, self).__init__()
        self.linear_q = nn.Linear(query_dim, ctx_dim, bias=False)
        self.linear_k = nn.Linear(ctx_dim, ctx_dim, bias=False)
        self.linear_v = nn.Linear(ctx_dim, ctx_dim, bias=False)
        self.dropout = nn.Dropout(attn_dropout)
        self.tanh = nn.Tanh()

    def forward(self, q, k, v, mask=None):

        q_ = self.linear_q(q)   # batch * seq_len * dim
        k_ = self.linear_k(k)   # batch * seq_len * dim
        v_ = self.linear_v(v)   # batch * seq_len * dim
        ...
        attn = self.dropout(F.softmax(attn, dim=-1))    #batch * seq_len * seq_len
...
class AttnDecoderLSTM(nn.Module):
    ...
    def forward(self, action, feature, cand_feat,
        ...
        h_tilde, sf_alpha = self.attention_layer(h_1_drop, ctx, ctx_mask)
        ...
        return h_1, c_1, logit, h_tilde, sf_alpha
...
class MeanPoolingCell(nn.Module):
    def __init__(self):
        super(MeanPoolingCell, self).__init__()

    def message_func(self, edges):
        return {'h': edges.src['h'], 'c': edges.src['c'], 'iou': edges.src['iou']}

    def reduce_func(self, nodes):
        return {'iou': torch.mean(nodes.mailbox['iou'], dim=1), 'h': torch.mean(nodes.mailbox['h'], dim=1), 'c': torch.mean(nodes.mailbox['c'], dim=1)}

    def apply_node_func(self, nodes):
        return {'h': nodes.data['iou'], 'c': nodes.data['iou']}
...
class TreeLSTM(nn.Module):
    def __init__(self,
        ...
        self.lstm = nn.LSTM(x_size, h_size, 1, batch_first=True, dropout=dropout, bidirectional=True)
        self.dropout = nn.Dropout(dropout)
        cell = TreeLSTMCell if cell_type == 'nary' else ChildSumTreeLSTMCell
        if args.lstm_layer == 2:
            self.cell = cell(h_size * 2, h_size)
        else:
            self.cell = cell(x_size, h_size)
        self.encoder2decoder = nn.Linear(h_size, h_size)
    ...
    def forward(self, batch, h, c, ctx_len, leaves_len, tree_type='none'):
        ...
        c : Tensor
            Initial cell state.
        """
        g = batch.to(torch.device('cuda'))
        embeds = self.embedding(batch.ndata['x'].cuda() * batch.ndata['leaf'].cuda())
        embeds = self.dropout(embeds)
        if args.lstm_layer == 2:
            _, dim = embeds.shape
            embeds = embeds.view(args.batchSize, -1, dim)
            _, max_len, _ = embeds.shape
            h0, c0 = self.init_state()
            packed_embeds = pack_padded_sequence(embeds, leaves_len, batch_first=True, enforce_sorted=False)
            enc_h, _ = self.lstm(packed_embeds, (h0, c0))
            ctx, _ = pad_packed_sequence(enc_h, batch_first=True, total_length=max_len)
            _, _, dim = ctx.shape
            ctx = ctx.view(-1, dim)
        else:
            ctx = embeds
        ...
        # Tree-LSTM
        g.ndata['iou'] = self.cell.W_iou(self.dropout(ctx)) * batch.ndata['leaf'].float().unsqueeze(-1).cuda()
        g.ndata['h'] = h.cuda()
        g.ndata['c'] = c.cuda()
        dgl.prop_nodes_topo(g, self.cell.message_func, self.cell.reduce_func, apply_node_func=self.cell.apply_node_func)

        h = self.dropout(g.ndata.pop('h'))
        c = self.dropout(g.ndata.pop('c'))
        g.ndata.pop('iou')

        # reshape for decoder
        _, dim = h.shape
        h = h.view(args.batchSize, -1, dim)
        _, max_len, dim = h.shape
        c = c.view(args.batchSize, -1, dim)
        if args.tree == 'none':
            g.ndata['root'] = batch.ndata['root'].float().cuda()
            r = g.ndata.pop('root')
            r = r.view(args.batchSize, -1)
            mask = r.unsqueeze(2).repeat(1, 1, dim)
        elif args.tree == 'dep':
            last_h_mask = (torch.arange(max_len, dtype=torch.int64).unsqueeze(0).repeat(args.batchSize, 1)
                           != (torch.LongTensor(np.array(leaves_len) - 1)).unsqueeze(1)).cuda()
            mask = torch.ones(h.shape).cuda().masked_fill_(last_h_mask.unsqueeze(2).repeat(1, 1, dim), 0)
        else:
            print("error")
            pass
        decoder_init_h = nn.Tanh()(self.encoder2decoder((h * mask).sum(1)))
        c_t = (c * mask).sum(1)

        return h, decoder_init_h, c_t

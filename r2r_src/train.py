...
if args.language == 'hi':
    TRAIN_VOCAB = 'tasks/RXR/data/train_vocab_hi.txt'
elif args.language == 'te':
    TRAIN_VOCAB = 'tasks/RXR/data/train_vocab_te.txt'
else:
...
def valid(train_env, tok, val_envs={}):
    ...
    iter = agent.load(args.load)
    print("Loaded the listener model at iter %d from %s" % (iter, args.load))
    ...
    for env_name, (env, evaluator) in val_envs.items():
        ...
        iters = None
        agent.test(use_dropout=False, feedback='argmax', iters=iters, training=False)
        ...
        if args.submit:
            json.dump(
                result,
                open(os.path.join(log_dir, "submit_%s_%s.json" % (env_name, args.language)), 'w'),
...
def setup():
    ...
    if not os.path.exists(TRAIN_VOCAB):
        if args.language == 'hi':
            write_vocab(build_vocab_hi(splits=['train']), TRAIN_VOCAB)
        elif args.language == 'te':
            write_vocab(build_vocab_te(splits=['train']), TRAIN_VOCAB)
        else:
...
def train_val():
    ...
    if args.submit:
        ...
        val_envs['train'] = (train_env, Evaluation(['train'], featurized_scans, tok))

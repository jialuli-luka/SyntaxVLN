...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if args.dataset == 'RXR':
            if split == 'test':
                split = 'test_standard_public'
            if args.tree == 'dep':
                if args.language == 'en':
                    with open('tasks/RXR/rxr-data/rxr_%s_guide_dep.jsonl' % split) as f:
                        new_data = json.load(f)
                elif args.language == 'hi':
                    with open('tasks/RXR/rxr-data/rxr_%s_guide_dep_hi.jsonl' % split) as f:
                        new_data = json.load(f)
                elif args.language == 'te':
                    with open('tasks/RXR/rxr-data/rxr_%s_guide_dep_te.jsonl' % split) as f:
                        new_data = json.load(f)
                else:
                    print("wrong language")
            else:
                if args.language == 'en':
                    with open('../RXR/rxr-data/rxr_%s_guide_dep.jsonl' % split) as f:
                        new_data = json.load(f)
                elif args.language == 'hi':
                    with open('../RXR/rxr-data/rxr_%s_guide_dep_hi.jsonl' % split) as f:
                        new_data = json.load(f)
                elif args.language == 'te':
                    with open('../RXR/rxr-data/rxr_%s_guide_dep_te.jsonl' % split) as f:
                        new_data = json.load(f)
                else:
                    print("wrong language")
        else:
            if "/" not in split:
                if args.tree == 'dep':
                    with open('tasks/%s/data/%s_%s_depparse.json' % (args.dataset, args.dataset, split)) as f:
                        new_data = json.load(f)
                else:
                    with open('tasks/%s/data/%s_%s.json' % (args.dataset, args.dataset, split)) as f:
                        new_data = json.load(f)
            else:
...
class Tokenizer(object):
    ...
    def encode_sentence_hite(self, parse, max_length=None):
        if max_length is None:
            max_length = self.encoding_length
        if len(self.word_to_index) == 0:
            sys.exit('Tokenizer has no vocab')
        ...
        for i in range(1, len(parse) + 1):
            encoding.append(self.word_to_index[parse[str(i)]['word']])
        encoding.append(self.word_to_index['<EOS>'])
    ...
    def parse(self, parse):
        parse_mask = np.array([False] * self.encoding_length)
        phrase = np.ones(self.encoding_length)
        # print(parse)
        parse = Tree.fromstring(parse)
        n_leaves = len(parse.leaves())
        leavepos = list(parse.leaf_treeposition(n) for n in range(n_leaves))
        tree_height = parse.height()
        bar = int(tree_height * args.level * 0.01)
        # print(bar)
        # bar = 0
        for i in range(0, bar + 1):
            parse_mask_tmp = np.array([False] * self.encoding_length)
            for pos in parse.treepositions():
                if pos not in leavepos:
                    if parse[pos].height() == i:
                        start_index = leavepos.index(pos + parse[pos].leaf_treeposition(0))
                        phrase_len = len(parse[pos].leaves())
                        if start_index >= self.encoding_length:
                            continue
                        if phrase_len != 1:
                            phrase[start_index] = phrase_len
                        for k in range(1, phrase_len):
                            if start_index + k >= self.encoding_length:
                                break
                            parse_mask_tmp[start_index + k] = True
            # print(parse_mask_tmp)
            parse_mask = np.logical_or(parse_mask, parse_mask_tmp)
        # print(parse_mask)


        unique_num = 0
        index = 0
        phrase_new = np.zeros(self.encoding_length)
        for i in range(self.encoding_length):
            if index >= self.encoding_length:
                break
            for j in range(int(phrase[index])):
                if index + j >= self.encoding_length:
                    break
                phrase_new[index + j] = unique_num
            unique_num += 1
            index += j + 1
        phrase_mask = np.zeros((self.encoding_length, self.encoding_length))
        for i in range(self.encoding_length):
            mask = np.ones(self.encoding_length)
            mask[i:i+int(phrase[i])] = 0
            phrase_mask[i, :] = mask

        return parse_mask, phrase_new, phrase_mask

    def encode_graph(self, parse):
        parse = Tree.fromstring(parse)
        n_leaves = len(parse.leaves())
        leaves = parse.leaves()
        ...
        def _rec_build(nid, node, pos):
            for p, child in enumerate(node):
                ...
                if not isinstance(child, Tree):
                    ...
                    pos = list(pos)
                    pos.append(p)
                    cid = pos_to_index[tuple(pos)]
                else:
                    if len(child) == 0:
                        continue
                    elif not isinstance(child[0], Tree):
                        cid = nid
                    elif isinstance(child[0], Tree) and len(child) == 1:
                        cid = nid
                    else:
                        if args.collapse and random.random() >= 0.9:
                            cid = nid
                        else:
                            g.add_node(cid, x=self.word_to_index['<PAD>'], leaf=0, npad=1)
                    pos = list(pos)
                    pos.append(p)
                    _rec_build(cid, child, pos)
                if cid != nid:
                    g.add_edge(cid, nid)
                pos.pop()
        ...
        for i, leave in enumerate(leaves):
            cid = g.number_of_nodes()
            g.add_node(cid, x=self.word_to_index[leave], leaf=1, npad=1)
            pos_to_index[parse.leaf_treeposition(i)] = cid

        # add root
        g.add_node(n_leaves, x=self.word_to_index['<PAD>'], leaf=0, npad=1)
        _rec_build(n_leaves, parse, ())
        ret = dgl.from_networkx(g, node_attrs=['x', 'leaf', 'npad'])

        return ret, n_leaves

    def encode_graph_dep(self, parse):
        n_words = len(parse)
        g = nx.DiGraph()
        if args.language == 'hi-IN' or args.language == 'te-IN':
            n_words += 1
        for i in range(1, n_words):
            if parse[str(i)]['head'] != 0:
                g.add_node(i - 1, x=self.word_to_index[parse[str(i)]['word']], leaf=1, npad=1, root=0)
            else:
                g.add_node(i - 1, x=self.word_to_index[parse[str(i)]['word']], leaf=1, npad=1, root=1)

        for i in range(1, n_words):
            if parse[str(i)]['head'] != 0:
                g.add_edge(i - 1, parse[str(i)]['head'] - 1)

        ret = dgl.from_networkx(g, node_attrs=['x', 'leaf', 'npad', 'root'])
        return ret, n_words - 1
